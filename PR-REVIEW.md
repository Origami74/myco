...
4. Project guidance. Read [README.md](README.md) at the repo root, the
   design docs under [docs/design/](docs/design/), and any nested
   `CLAUDE.md` / `AGENTS.md` in directories the diff touches. These
   describe project-specific conventions and constraints not visible
   from the diff alone.
5. Related work on GitHub. Skim the [open issues](https://github.com/Origami74/myco/issues)
   and other [open PRs](https://github.com/Origami74/myco/pulls) for
...
9. **Documentation impact**. Does this need a [CHANGELOG.md](CHANGELOG.md)
   entry, rustdoc / KDoc updates, design-doc changes
   ([docs/design/](docs/design/)), README adjustments, or other
   contributor docs under [docs/](docs/)?
...
11. **Language best practices (Rust + Kotlin/Android)**. *Rust*
    (`myco-core`, `myco-relay`, `myco-blossom`, `nsite-deck`):
    idiomatic error handling, no silently-swallowed errors, no
    `unwrap` / `expect` on untrusted input, no `#[allow]` without
    justification, appropriate visibility (`pub` vs `pub(crate)` vs
    private), naming, and module shape. *Kotlin / Android* (`android/`):
    null-safety; structured concurrency (coroutines scoped to a
    lifecycle, nothing blocking the main thread — IO and JNI/FFI calls
    off the UI thread); Compose state hygiene (no side effects in
    composition, correct `remember`/effect keys, `DisposableEffect`
    cleanup); lifecycle-correct teardown of services and BLE/NFC
    handles; and careful runtime-permission / intent handling. At the
    JNI/FFI boundary (`myco-core` ↔ app) watch handle lifetime,
    threading, and error propagation across the bridge.
...
13. **Other concerns**. Anything not captured above — on-mesh
    wire-format / protocol implications (pair-event kinds, relay +
    Blossom access gating, gossip), Android packaging impact (manifest,
    permissions, `minSdk` 29, the cargo → `jniLibs` native build),
    contributor coordination needs, and fragility notes for future
    maintainers.
...
- Things explicitly silenced by an `#[allow]` / `@Suppress` with
  justification
- Stylistic preferences not anchored in the project docs or the
...
```text
https://github.com/Origami74/myco/blob/<full-40-char-sha>/<path>#L<start>-L<end>
```
