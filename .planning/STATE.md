...
- [Phase ?]: 01-03: the BLE attempt log lives in fips as a process-global singleton reached by a free accessor, mirroring android_io.rs's bridge accessor — threading it through accept_loop/scan_probe_loop was rejected as a larger, harder-to-extract upstream diff
...
- **Device verification batched:** six checks needing physical phones are collected in
  `.planning/phases/01-make-peering-observable/DEVICE-TEST-BATCH.md`. The highest-value one
  (D-1) reads the 01-03 attempt log to settle the tiebreaker-race hypothesis Phase 2 must not
  act on by inference.
