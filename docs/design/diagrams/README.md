...
| [02-pairing-transitive-discovery.svg](02-pairing-transitive-discovery.svg) | Mutual pairing — now **invite-pairing** (a one-time secret; post-v1, supersedes mutual scan) — authorizes polling a peer's collected list, so Alice transitively reaches Ben + Ben's peers. (A one-way scan stays fetch-only.) |
