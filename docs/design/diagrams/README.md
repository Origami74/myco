...
| [intro-02-get-started.svg](intro-02-get-started.svg) | The 3 steps: scan a friend's QR → install their apps → open one (works offline). **⚠ redraw pending** — depicts a one-way "scan a friend's QR"; doesn't yet reflect handshake-mandatory, always-mutual pairing. |
...
| [07-relay-mesh-fanout.svg](07-relay-mesh-fanout.svg) | Event fanout: the **nsite-deck propagator** (not the relay) re-broadcasts each accepted event to all *other* connected peers (source-excluded, deduped); events fan out, blobs stay pull-only. **⚠ redraw pending** — still labels the embedded *relay* as the re-broadcaster; fanout is now a separate propagator process that subscribes + publishes. |
